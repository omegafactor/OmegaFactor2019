...
public class MecanumDrive extends RobotDriveBase {
  ...
  private final BaseMotorController m_frontLeftMotor;
  private final BaseMotorController m_rearLeftMotor;
  private final BaseMotorController m_frontRightMotor;
  private final BaseMotorController m_rearRightMotor;
   ...
   */
  public MecanumDrive(BaseMotorController frontLeftMotor, BaseMotorController rearLeftMotor,
  ...
  private void verify(BaseMotorController frontLeft, BaseMotorController rearLeft,
  BaseMotorController frontRight, BaseMotorController rearRightMotor) {
  ...
  //public void driveCartesian(double ySpeed, double xSpeed, double zRotation, double leftPower, double rightPower) {
   // driveCartesian(ySpeed, xSpeed, zRotation, 0.0, leftPower, rightPower);
 // }
   ...
   */
  public void strafeDrive(double ySpeed, double xSpeed, double zRotation, double gyroAngle){
    if (!m_reported) {
      ...
      double m_maxOutputLeft = 1;
      double m_maxOutputRight = 1;
      m_frontLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontLeft.value] * m_maxOutputLeft);
      m_frontRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontRight.value] * m_maxOutputRight
          * m_rightSideInvertMultiplier);
      m_rearLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearLeft.value] * m_maxOutputLeft);
      m_rearRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearRight.value] * m_maxOutputRight
          * m_rightSideInvertMultiplier);
  
      feed();
  }
  @SuppressWarnings("ParameterName")
  public void turning(double ySpeed, double xSpeed, double zRotation, double gyroAngle) {
    if (!m_reported) {
      HAL.report(tResourceType.kResourceType_RobotDrive, 4,
                 tInstances.kRobotDrive2_MecanumCartesian);
      m_reported = true;
    ...
    m_frontLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontLeft.value]);
    m_frontRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontRight.value]
        * m_rightSideInvertMultiplier);
    m_rearLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearLeft.value] );
    m_rearRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearRight.value] 
        * m_rightSideInvertMultiplier);
  ...
  public void fwbw(double ySpeed, double xSpeed, double zRotation, double gyroAngle, double leftPower, double rightPower) {
    if (!m_reported) {
      HAL.report(tResourceType.kResourceType_RobotDrive, 4,
                 tInstances.kRobotDrive2_MecanumCartesian);
      m_reported = true;
    ...
    double[] wheelSpeeds = new double[4];
    wheelSpeeds[MotorType.kFrontLeft.value] =  input.y + input.x + zRotation;
    wheelSpeeds[MotorType.kFrontRight.value] = input.y - input.x - zRotation;
    wheelSpeeds[MotorType.kRearLeft.value] = input.y - input.x + zRotation;
    wheelSpeeds[MotorType.kRearRight.value] = input.y + input.x - zRotation;

    normalize(wheelSpeeds);

    m_frontLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontLeft.value] * leftPower);
    m_frontRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kFrontRight.value] * rightPower
        * m_rightSideInvertMultiplier);
    m_rearLeftMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearLeft.value] * leftPower);
    m_rearRightMotor.set(ControlMode.PercentOutput,wheelSpeeds[MotorType.kRearRight.value] * rightPower
        * m_rightSideInvertMultiplier);
   ...
   */
  @SuppressWarnings("ParameterName")
  public void drivePolar(double magnitude, double angle, double zRotation, double leftPower, double rightPower) {
    if (!m_reported) {
    ...
    //driveCartesian(magnitude * Math.sin(angle * (Math.PI / 180.0)),
                 //  magnitude * Math.cos(angle * (Math.PI / 180.0)), zRotation, 0.0, leftPower, rightPower);
  }
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("Front Right Motor Speed",
        ...
        value -> m_frontRightMotor.set(ControlMode.PercentOutput, value * m_rightSideInvertMultiplier));
    builder.addDoubleProperty("Rear Left Motor Speed",
        m_rearLeftMotor::get,
        m_rearLeftMotor::set);
    builder.addDoubleProperty("Rear Right Motor Speed",
        () -> m_rearRightMotor.get() * m_rightSideInvertMultiplier,
        value -> m_rearRightMotor.set(ControlMode.PercentOutput, value * m_rightSideInvertMultiplier));*/
  }
}
